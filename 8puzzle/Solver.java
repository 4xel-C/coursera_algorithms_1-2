...
public class Solver {
    ...
    public Solver(Board initial) {
        ...
        if (initial == null) throw new IllegalArgumentException("Board to solve cannot be null");
        ...
        while (true) {
            ...
            // if solution if found in twinNode -> no solution
                ...
                for (Board board: node.board.neighbors()) {
                    ...
                    if (board == node.board) continue;
                    
                    // Insert each new node updating the values depending of the current search node.
                    pq.insert(new SearchNode(board, node.moves + 1, node));
                }
            }
            
            // Iterate throughout all neighbors of the twinNode and add items to the respective pq
            for (Board board: twinNode.board.neighbors()) {
                
                // Avoid adding the same board to the pq
                if (board == twinNode.board) continue;
    ...
    public Iterable<Board> solution() {
        ...
        // Initialize a stack storing all solutions from the last node up to the first node
